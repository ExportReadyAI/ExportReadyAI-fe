...
import type { UpdateBuyerRequestRequest, Country, BuyerRequest } from "@/lib/api/types"
...
export default function EditBuyerRequestPage() {
  ...
  const [formData, setFormData] = useState<UpdateBuyerRequestRequest>({
    product_category: "",
    hs_code_target: "",
    spec_requirements: "",
    target_volume: 0,
  ...
  useEffect(() => {
    if (!isAuthenticated || !isBuyer() || !requestId) {
      router.push("/login")
      return
    }
    ...
    fetchCountries()
    fetchRequest()
  }, [isAuthenticated, router, isBuyer, requestId])
  ...
  const fetchRequest = async () => {
    try {
      ...
      const request = requestData as BuyerRequest

      setFormData({
        product_category: request.product_category || "",
        hs_code_target: request.hs_code_target || "",
        spec_requirements: request.spec_requirements || "",
        target_volume: request.target_volume || 0,
        destination_country: request.destination_country || "",
        keyword_tags: request.keyword_tags || [],
        min_rank_required: request.min_rank_required || 0,
      })
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload: UpdateBuyerRequestRequest = {
      ...
      await buyerRequestService.update(requestId, payload)
      router.push(`/buyer-requests/${requestId}`)
    } catch (err: any) {
      setError(
        err.response?.data?.message ||
        err.message ||
        "Terjadi kesalahan saat mengupdate buyer request"
      )
    } finally {
      setLoading(false)
    }
  ...
  if (!isAuthenticated || !isBuyer() || loadingData) {
    return null
  }
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="container mx-auto px-8 py-8">
          ...
          <div className="mb-6">
            <Button
              variant="ghost"
              onClick={() => router.push(`/buyer-requests/${requestId}`)}
              className="mb-4"
            >
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[#0284C7] font-medium">
                  Edit permintaan pembelian Anda
                </p>
              </div>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <CardContent className="space-y-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    placeholder="Contoh: Makanan Olahan, Tekstil, dll"
            ...
            <div className="mt-6 flex justify-end gap-4">
              ...
              >
                {loading ? "Mengupdate..." : "Update Request"}
              </Button>
            </div>
          </form>
        </div>
